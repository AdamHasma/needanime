<script>
  import { mangaFormats, stripHtml } from "../lib/anilist.js";
  import AnimeModal from "./AnimeModal.svelte";

  let { anime } = $props();

  // how much of a novel is out already — anilist reports a volume count for
  // most finished novels, a chapter count for some, and nothing for the rest
  const published = $derived(
    anime.volumes
      ? `${anime.volumes} volumes`
      : anime.chapters
        ? `${anime.chapters} chapters`
        : mangaFormats.has(anime.format) && anime.status === "RELEASING"
          ? "ongoing"
          : null
  );
...
>
  ...
  <div class="flex flex-wrap items-center gap-x-4 gap-y-1 border-b border-line p-3 text-sm text-text-muted">
    ...
    {#if published}
      <span class="flex items-center gap-1.5" title="Published so far">
